...
void Xsift(const cv::Mat img,std::vector<cv::KeyPoint>& keypoints,cv::Mat descriptors,
	int nOctaveLayers,double contrastThreshold,float edgeThreshold,float sigma )
{
	Mat dst;
	vector<Mat> gaussianPyr,dogPyr;
	xsift::CreateInitialImage(img,dst,SIFT_IMG_DBL);
	int firstOctave=SIFT_IMG_DBL?-1:0;
	int nOctaves=(int)(log((double)std::min(img.cols,img.rows))/log(2.)-2-firstOctave);
	xsift::BuildGaussianPyramid(dst,gaussianPyr,nOctaves);
	xsift::BuildDoGPyramid(gaussianPyr,dogPyr,nOctaves);
	//showPyr(gaussianPyr,nOctaves,6);
	writePyr(gaussianPyr,nOctaves,6,"F:\\sift\\gaussian",false);
	writePyr(dogPyr,nOctaves,5,"F:\\sift\\dog_nostretch",true);
	writePyrValue(dogPyr,nOctaves,5,"F:\\sift\\dog_value");
	xsift::FindSpaceScaleExtrema(dogPyr,gaussianPyr,keypoints,nOctaves);
	xsift::AdjustByInitialImage(keypoints,firstOctave);
	xsift::CalcDescritors(gaussianPyr,keypoints,descriptors,firstOctave);


	//显示所有阶段的特征点
//	vector<KeyPoint> interpKeypoints,finalKeypoints;
//	xsift::FindSpaceScaleExtrema(dogPyr,keypoints,interpKeypoints,finalKeypoints,nOctaves);
//	xsift::AdjustByInitialImage(keypoints,firstOctave);
//	xsift::AdjustByInitialImage(interpKeypoints,firstOctave);
//	xsift::AdjustByInitialImage(finalKeypoints,firstOctave);
//	Mat src;
